...
import { createD26OfflineHarness } from "./support/d26-offline-controlled-encoding-failure-harness.js";

describe("D26 Controlled encoding failure offline tests", () => {
  const harness = createD26OfflineHarness();

  it("returns ENCODING_FAILED when controlled encoding failure is injected", async () => {
    const record = await harness.encodingFailed();
    assert.equal(record.testCaseId, "D26_CONTROLLED_ENCODING_FAILURE");
    assert.equal(record.status, "FAIL");
    assert.equal(record.errorCode, "ENCODING_FAILED");
  ...
  it("returns UNEXPECTED_RESULT when error code does not match", async () => {
    ...
    assert.equal(record.expectedErrorCode, "ENCODING_FAILED");
