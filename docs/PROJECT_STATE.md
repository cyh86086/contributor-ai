...
- D26 evidence-gap review, device-procedure preparation, and scoped device validation are complete. The D26 contract verifies that when a controlled encoding failure is injected after a valid read, the portable core returns the stable public error code `ENCODING_FAILED` with a frozen, sanitized failure record. Offline tests (5/5 pass) cover ENCODING_FAILED, UNEXPECTED_RESULT, HARNESS_EXCEPTION, and input validation. Device validation is pending on Vivo X Fold5 / AutoJs6 v6.7.0 `arm64-v8a`. The review, procedure, and offline tests are recorded in [`testing/d26-controlled-encoding-failure-evidence-gap-review.md`](testing/d26-controlled-encoding-failure-evidence-gap-review.md), [`user-guides/autojs6-d26-controlled-encoding-failure-check-zh-tw.md`](user-guides/autojs6-d26-controlled-encoding-failure-check-zh-tw.md), and [`tests/autojs6-d26-controlled-encoding-failure.test.js`](../../tests/autojs6-d26-controlled-encoding-failure.test.js).
...
At this snapshot it is `None`. D22 and D23 device validations both completed with scoped PASS on Vivo X Fold5 / Android 16 / AutoJs6 v6.7.0 `arm64-v8a`.
