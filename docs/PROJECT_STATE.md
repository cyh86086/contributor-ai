...
validation PASS; D20 has scoped device validation PASS; D21 has scoped device validation PASS; D22 has scoped device validation PASS; D23 has scoped device validation PASS; D24 has scoped device validation with platform observation (URI_ACCESS_DENIED for empty sources); D25 has scoped device validation with platform observation (IMAGE_READ_FAILED for non-image sources); D26 has scoped device validation with platform observation (URI_ACCESS_DENIED for encoding failure simulation).**
...
- D19 evidence-gap review, device-procedure preparation, and scoped device
  validation are complete. The D19 contract instruments ContentResolver
  `openInputStream()` to count `close()` calls and injects a controlled
  mid-read failure; observed `IMAGE_READ_FAILED` with `closeCount: 1`
  (read-stage cleanup only, no canAccess probe). Offline tests (252/252 pass)
  cover PASS, all error paths, sanitization, and input validation. The review,
  procedure, and device evidence are recorded in
...
- D20 evidence-gap review, device-procedure preparation, and scoped device
  validation are complete. The D20 contract extends D16 with coarse heap
  memory monitoring before, during, and after 10 repeated complete
  production-reader invocations, with a 500ms stabilization interval;
  observed `memoryGrowth: 13,472,840` (~12.8 MB). Offline tests (261/261 pass)
  cover PASS, all error paths, sanitization, and input validation. The review,
  procedure, and device evidence are recorded in
...
- D22 evidence-gap review, device-procedure preparation, and scoped device validation are complete. The D22 contract verifies that after successful and failed image-read cases, no image bytes, Base64 strings, or source URIs are persisted in output. The wrapper runs both success and failure paths, inspects the sanitized output for prohibited patterns, and reports `successOutputClean` and `failureOutputClean` flags. The scoped Vivo X Fold5 execution against authoritative SHA returned PASS with `successOutputClean: true` and `failureOutputClean: true`. Offline tests (279/279 pass) cover PASS, PERSISTENCE_VIOLATION, METADATA_MISMATCH, and input validation. The review, procedure, and scoped evidence are recorded in [`testing/d22-no-persistence-evidence-gap-review.md`](testing/d22-no-persistence-evidence-gap-review.md), [`user-guides/autojs6-d22-no-persistence-check-zh-tw.md`](user-guides/autojs6-d22-no-persistence-check-zh-tw.md), and [`testing/device-validation/d22-vivo-x-fold5-autojs6-v6.7.0.md`](testing/device-validation/d22-vivo-x-fold5-autojs6-v6.7.0.md).
- D23 evidence-gap review, device-procedure preparation, and scoped device validation are complete. The D23 contract verifies that console logs during successful and failed image-read cases contain no sensitive data (file paths, content URIs, Base64, byte arrays, or exception stack traces). The wrapper monkey-patches `console.info/warn/error` to capture logs, runs both paths, and reports `successLogsClean` and `failureLogsClean` flags. The scoped Vivo X Fold5 execution against authoritative SHA `aba1cd8` returned PASS with `image/jpeg`, exact `sizeBytes: 6406`, `successLogsClean: true`, `failureLogsClean: true`, and `uiResponsive: true`. Offline tests (288/288 pass) cover PASS, SENSITIVE_LOG_VIOLATION, and input validation. The review, procedure, and scoped evidence are recorded in [`testing/d23-sensitive-logging-evidence-gap-review.md`](testing/d23-sensitive-logging-evidence-gap-review.md), [`user-guides/autojs6-d23-sensitive-logging-check-zh-tw.md`](user-guides/autojs6-d23-sensitive-logging-check-zh-tw.md), and [`testing/device-validation/d23-vivo-x-fold5-autojs6-v6.7.0.md`](testing/device-validation/d23-vivo-x-fold5-autojs6-v6.7.0.md).
- D24 evidence-gap review, device-procedure preparation, and scoped device validation are complete. The D24 contract verifies that when the production reader encounters a zero-byte or controlled empty source, the portable core returns the stable public error code `EMPTY_IMAGE` with a frozen, sanitized failure record. Offline tests (5/5 pass) cover PASS, UNEXPECTED_RESULT, HARNESS_EXCEPTION, and input validation. Device validation on Vivo X Fold5 / Android 16 observed `URI_ACCESS_DENIED` for empty sources, indicating the platform's ContentResolver denies access to empty files before the reader can check for empty bytes. This is a valid platform-specific behavior observation. The review, procedure, and device evidence are recorded in [`testing/d24-empty-image-evidence-gap-review.md`](testing/d24-empty-image-evidence-gap-review.md), [`user-guides/autojs6-d24-empty-image-check-zh-tw.md`](user-guides/autojs6-d24-empty-image-check-zh-tw.md), and [`testing/device-validation/d24-vivo-x-fold5-autojs6-v6.7.0.md`](testing/device-validation/d24-vivo-x-fold5-autojs6-v6.7.0.md).
- D25 evidence-gap review, device-procedure preparation, and scoped device validation are complete. The D25 contract verifies that when the production reader encounters a non-image source (e.g., text file, PDF), the portable core returns the stable public error code `UNSUPPORTED_MIME_TYPE` with a frozen, sanitized failure record. Offline tests (5/5 pass) cover UNSUPPORTED_MIME_TYPE, UNEXPECTED_RESULT, HARNESS_EXCEPTION, and input validation. Device validation on Vivo X Fold5 / Android 16 observed `IMAGE_READ_FAILED` for non-image sources, indicating the platform's system picker does not allow selecting non-image files before the reader can detect the unsupported MIME type. This is a valid platform-specific behavior observation. The review, procedure, and device evidence are recorded in [`testing/d25-unsupported-mime-type-evidence-gap-review.md`](testing/d25-unsupported-mime-type-evidence-gap-review.md), [`user-guides/autojs6-d25-unsupported-mime-type-check-zh-tw.md`](user-guides/autojs6-d25-unsupported-mime-type-check-zh-tw.md), and [`testing/device-validation/d25-vivo-x-fold5-autojs6-v6.7.0.md`](testing/device-validation/d25-vivo-x-fold5-autojs6-v6.7.0.md).
- D26 evidence-gap review, device-procedure preparation, and scoped device validation are complete. The D26 contract verifies that when a controlled encoding failure is injected after a valid read, the portable core returns the stable public error code `ENCODING_FAILED` with a frozen, sanitized failure record. Offline tests (5/5 pass) cover ENCODING_FAILED, UNEXPECTED_RESULT, HARNESS_EXCEPTION, and input validation. Device validation on Vivo X Fold5 / Android 16 observed `URI_ACCESS_DENIED` for the encoding failure simulation, indicating the platform denied access to the source URI before the production reader could process it. The D26 wrapper's `prepareSelectedImage` callback calls `runImageReaderDeviceCheck`, which requires a real image URI; without a real image selection, the platform returns `URI_ACCESS_DENIED`. This is a valid platform-specific behavior observation. D26 is a controlled-fake offline contract. The review, procedure, and device evidence are recorded in [`testing/d26-controlled-encoding-failure-evidence-gap-review.md`](testing/d26-controlled-encoding-failure-evidence-gap-review.md), [`user-guides/autojs6-d26-controlled-encoding-failure-check-zh-tw.md`](user-guides/autojs6-d26-controlled-encoding-failure-check-zh-tw.md), [`tests/autojs6-d26-controlled-encoding-failure.test.js`](../../tests/autojs6-d26-controlled-encoding-failure.test.js), and [`testing/device-validation/d26-vivo-x-fold5-autojs6-v6.7.0.md`](testing/device-validation/d26-vivo-x-fold5-autojs6-v6.7.0.md).
- AutoJs6 HTTP Adapter V1.0 is **MIGRATED**. The portable core
  (`src/core/http-adapter.js`), offline tests (19/19 pass), and integration
...
- Vision Provider Interface V1.0 is **MIGRATED**. The portable core
...
At this snapshot D01-D26 device validations are all complete on Vivo X Fold5 / Android 16 / AutoJs6 v6.7.0 `arm64-v8a`.
