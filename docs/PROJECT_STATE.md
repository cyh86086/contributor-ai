...
validation PASS; D20 has scoped device validation PASS; D21 has scoped device validation PASS; D22 has scoped device validation PASS; D23 has scoped device validation PASS; D24 preparation complete, awaiting device validation; D25-D26 remain pending.**
...
- D24 evidence-gap review and device-procedure preparation are complete. The D24 contract verifies that when the production reader encounters a zero-byte or controlled empty source, the portable core returns the stable public error code `EMPTY_IMAGE` with a frozen, sanitized failure record. Offline tests (5/5 pass) cover PASS, UNEXPECTED_RESULT, HARNESS_EXCEPTION, and input validation. The review is recorded in [`testing/d24-empty-image-evidence-gap-review.md`](testing/d24-empty-image-evidence-gap-review.md). Device validation feasibility depends on whether the Android system picker permits selecting a 0-byte file; if not, D24 becomes a controlled-fake offline contract only.
